...
const usePeople = ({ activeFilter = SONGS_FILTER[0] }: IProps = {}) => {
  ...
  // Debug: show fetched people in browser console
  if (typeof window !== 'undefined') {
    if (data) console.log('[usePeople] fetched people count:', peopleData.length, data);
    if (error) console.error('[usePeople] error:', error);
  }

...
import useFirstItems from '@/hooks/use-first-items';
...
const Hero = ({ isSearchOpen }: IHeroProps) => {
  ...
  // get first_items (upstream home payload) — this will log to browser console via hook
  const { data: firstItems, isLoading: firstItemsLoading } = useFirstItems();

  // debug: show first items in browser console (help user confirm payload)
  if (typeof window !== 'undefined') {
    if (firstItems) console.log('[Hero] first_items', firstItems);
  }
        ...
        <div className="relative z-10 pr-0 pl-0 sm:px-6  pt-26 home-hero-container">
          ...
          {!isLoading && (
            <div className="columns-1 md:columns-2 lg:columns-1 gap-6  pt-8">
              {/* ✅ Prefer first_items.song from upstream if present, otherwise use visibleSongs */}
              {firstItems?.song ? (
                <div key={firstItems.song.id} className={`break-inside-avoid mb-6 product-card py-0.5 ${!(firstItems.song.youtubeVideoId || firstItems.song.thumbnailURL) ? 'no-media-padding' : ''}`}>
                  <div className="home-song-card">
                    <SongCard {...(firstItems.song as any)} />
                  </div>
                </div>
              ) : (
